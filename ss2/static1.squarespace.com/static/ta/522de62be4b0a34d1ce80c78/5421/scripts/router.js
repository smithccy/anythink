  ...
  Y.Router = Y.extend(Router, Y.Base, {
    ...
    **/
    ...
    initializer: function(config) {
      ...
      self._setRoutes(config && config.routes ? config.routes : self.get('routes'));

      // Set up a history instance or hashchange listener.
      if (self._html5) {
        self._history = new Y.HistoryHTML5({
          force: true
        });
        self._historyEvents = Y.after('history:change', self._afterHistoryChange, self);
      } else {
        self._historyEvents = Y.on('hashchange', self._afterHistoryChange, win, self);
      }
    ...
    **/
    _getPath: function() {
      var path = (!this._html5 && this._getHashPath()) || Y.getLocation().pathname;
    ...
    **/
    _joinURL: function(url) {
      ...
      return root && root.charAt(root.length - 1) === '/' ? root + url : root + '/' + url;
    },
    ...
    **/
    _afterHistoryChange: function(e) {
      ...
      // this `popstate` event.
      if (src === 'popstate' && (!self._ready || prevURL.replace(/#.*$/, '') === currentURL.replace(/#.*$/, ''))) {
